...
/* === 选择保护：防止 mouseup 触发 hover/click 破坏选区 === */
let isMouseDownInCard = false;
let hoverSuppressTimer = null;
...
function setHoverSuppressed(on) {
    ...
    notesContainer.classList.toggle('suppress-hover', !!on);
}

function installSelectionGuards() {
    if (!notesContainer) return;

    // 仅当在卡片正文内按下时，进入“选择可能发生”状态
    notesContainer.addEventListener('mousedown', (e) => {
        isMouseDownInCard = !!e.target.closest('.note-card .note-content');
    });

    // 释放鼠标：如存在选区，则短暂屏蔽 hover，并拦截随后的 click
    document.addEventListener('mouseup', () => {
        if (!isMouseDownInCard) return;
        isMouseDownInCard = false;
        if (isTextSelected()) {
            setHoverSuppressed(true);
            clearTimeout(hoverSuppressTimer);
            hoverSuppressTimer = setTimeout(() => setHoverSuppressed(false), 250);
        }
    });

    // 捕获阶段拦截 click，确保不触发任何卡片点击逻辑
    document.addEventListener('click', (e) => {
        if (!e.target) return;
        if (e.target.closest('.note-card') && isTextSelected()) {
